...
export default async function (app) {
    app.post("/send", {
        ...
        const conversation = await conversationManager.getConversationBySecret(secret);
        if (!conversation) {
            return reply.code(404).send({ message: "Conversation not found" });
        }

        try {
            const response = await conversation.send(message, context, playerId);
            if (!response) {
                return reply.code(429).send({ message: "Conversation is busy" });
            }
